...
@Component({
  ...
  templateUrl: './settings-finance.component.html',
  styleUrls: ['./settings-finance.component.css']
})
export class SettingsFinanceComponent implements OnInit {
	windowHt: any;
	...
	settingsHoverAction: any[] = [{hover: false, selected: false, route: 'payments'}, {hover: false, selected: false, route: 'payments'}, {hover: false, selected: false, route: 'insurance'}, {hover: false, selected: false, route: 'payments'}, {hover: false, selected: false, route: 'postingcodes'}, {hover: false, selected: false, route: 'payments'}, {hover: false, selected: false, route: 'payments'}, {hover: false, selected: false, route: 'payments'}];
  ...
  selectMenu(indx){
	...
	this.router.navigate(['../finances', {mnid: indx}], {relativeTo: this.activatedRoute}).then(() => {
  ...
  openTab(tab){
	if(tab == 'clinic'){
		this.router.navigate(['../clinic', {mnid: 0}], {relativeTo: this.activatedRoute});
	} else if(tab == 'operations'){
		this.router.navigate(['../operations', {mnid: 0}], {relativeTo: this.activatedRoute});
	} else if(tab == 'finance'){
		this.messageService.sendMessage('backtoparent', {section: 'finances'});
